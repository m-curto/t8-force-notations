...
int KBL_build(HWND &hwnd, AppState &app)
{
    int total = 0;
    ...
    std::vector<fs::path> specific = {BMP_BG_KB, "resources/none.bmp"};

    for (int s = 0; s < BIND_MAX; ++s) // TODO BIND_MAX / XSX_MAX
    {
        ...
        if (btn > 0 && cmd > 0) {
            ...
            fs::create_directories(dst); std::cout << "CREATE_DIR\t" << dst.string() << std::endl;
            app.buildBar.step();
...
{
    if (c == NUM_1 || c == NUM_DARK_1 || c == NUM_VERTICAL_1 || c == DEFAULT_1 || c == DEFAULT_DARK_1 || c == PLAYSTATION_1 || c == PLAYSTATION_DARK_1 || c == PLAYSTATION_VERTICAL_1 || c == PLAYSTATION_VERTICAL_DARK_1 || c == XBOX_1 || c == XBOX_DARK_1) return "1";
    if (c == NUM_2 || c == NUM_DARK_2 || c == NUM_VERTICAL_2 || c == DEFAULT_2 || c == DEFAULT_DARK_2 || c == PLAYSTATION_2 || c == PLAYSTATION_DARK_2 || c == PLAYSTATION_VERTICAL_2 || c == PLAYSTATION_VERTICAL_DARK_2 || c == XBOX_2 || c == XBOX_DARK_2) return "2";
    if (c == NUM_3 || c == NUM_DARK_3 || c == NUM_VERTICAL_3 || c == DEFAULT_3 || c == DEFAULT_DARK_3 || c == PLAYSTATION_3 || c == PLAYSTATION_DARK_3 || c == PLAYSTATION_VERTICAL_3 || c == PLAYSTATION_VERTICAL_DARK_3 || c == XBOX_3 || c == XBOX_DARK_3) return "3";
    if (c == NUM_4 || c == NUM_DARK_4 || c == NUM_VERTICAL_4 || c == DEFAULT_4 || c == DEFAULT_DARK_4 || c == PLAYSTATION_4 || c == PLAYSTATION_DARK_4 || c == PLAYSTATION_VERTICAL_4 || c == PLAYSTATION_VERTICAL_DARK_4 || c == XBOX_4 || c == XBOX_DARK_4) return "4";
    if (c == ASSIST) return "assist";
    if (c == BLANK) return "blank";
    if (c == NOSELECT) return "noselect";
    if (c == RAGE) return "rage";
...
int preset(AppState &app, const int &c) {
    ...
    if (app.layout_current == LAYOUT_KB)
    {
        if (c == 0) { // "Keyboard Settings 1"
            ...
            app.KB[3].setCursor(KC_D);          app.notations[3].setCursor(XBOX_DARK_1);            app.KB[15].setCursor(KC_4);     app.notations[15].setCursor(NONE);
